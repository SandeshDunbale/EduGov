...
public class ProgramServiceImpl implements ProgramService {
	...
	public List<ProgramDTO> searchPrograms(String title) {
		...
		return list.stream().map(this::mapToCustomDto).collect(Collectors.toList());
	}
	...
	public ProgramDTO updateProgramById(Long id, Program details) {
		...
		// 1. Title/Status/Desc Validations
		...
		if (details.getStartDate() != null) {
			if (details.getStartDate().isBefore(LocalDate.now())) {
				throw new APIException(HttpStatus.BAD_REQUEST, "Start date must be today or in the future.");
			}
		}
