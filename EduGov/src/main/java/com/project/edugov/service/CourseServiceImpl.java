...
public class CourseServiceImpl implements CourseService {
	...
	public CourseDTO createCourse(Course course, Long pId, Long fId, Long aId) {
		...
		User user1=faculty.getUser();
		notificationService.createNotification(
				fId,
				aId,
				"Course Alloted to You.Please login and go through it",
				"ACADEMICS",
				user1.getEmail()
				);
	...
	public List<CourseDTO> getCoursesByFacultyId(Long facultyId) {
		...
		// 4. Map to DTO
		return courses.stream().map(c -> mapToCustomDto(c, c.getUser())).toList();
	}
	...
	public CourseDTO updateCourse(Long id, Course details) {
		...
		if (details.getTitle() != null) {
			...
			if (!newTitle.equalsIgnoreCase(existing.getTitle()) && courseRepo
					.existsByTitleIgnoreCaseAndProgram_ProgramID(newTitle, existing.getProgram().getProgramID())) {
		...
		if (details.getStatus() != null && !details.getStatus().equals(Status.ACTIVE)
				&& details.getStatus().equals(existing.getStatus())) {
			throw new APIException(HttpStatus.BAD_REQUEST, "Update failed: Status is already " + existing.getStatus());
		}
