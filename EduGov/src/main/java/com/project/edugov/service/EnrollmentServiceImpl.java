...
public class EnrollmentServiceImpl implements EnrollmentService {
	...
	public List<EnrollmentResponseDTO> getEnrollmentsByStatus(Status status) {
		...
		return enrollments.stream().map(this::mapToCustomDto).toList();
	}
	...
	public EnrollmentResponseDTO updateEnrollmentStatus(Long enrollmentId, Long adminId, Status newStatus) {
		...
		if (updatedEnrollment.getStudent() != null && updatedEnrollment.getStudent().getUser() != null) {
			User user = updatedEnrollment.getStudent().getUser();
			
			// 2. Make the message dynamic based on the newStatus
			String message = "Your course enrollment status has been updated to: " + newStatus;
			if (newStatus == Status.APPROVE) {
			    message = "Congratulations! Your course enrollment has been Approved.";
			} else if (newStatus == Status.REJECT) {
			    message = "We're sorry, but your course enrollment has been Rejected.";
			}

			// 3. Send the notification
			notificationService.createNotification(
					user.getUserId(),
					updatedEnrollment.getEnrollmentID(), // Using the enrollment ID as the entityId
					message,
					"ACADEMICS",
					user.getEmail()
			);
		} else {
			log.warn("Notification skipped: No user attached to Enrollment ID {}", enrollmentId);
		}
	...
	public List<EnrollmentResponseDTO> getAllEnrollments() {
		...
		return allEnrollments.stream().map(this::mapToCustomDto).toList();
	}
}